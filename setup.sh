...
GROUP_KEYS=(tools)
GROUP_DESC=(
    "[Tools] four agent-loops (including pi-agent-loop), cleanup, gotcha"
)
...
说明：
  ...
  - 安装 capture-gotcha 时会初始化 ~/.agents/gotchas.md。
  - 三个 agent 均使用扁平结构。
EOF
}
...
skill_group() {
    case "$1" in
        cleanup|agent-loop|capture-gotcha|cc-agent-loop|codex-agent-loop|hermes-agent-loop|pi-agent-loop) echo "tools" ;;
        *) echo "" ;;
    esac
}
...
ensure_gotchas() {
    ...
    local master="$HOME/.agents/gotchas.md"
    if [ "$DRY_RUN" -eq 1 ]; then
        echo "预览：将初始化 $master"
        return 0
    fi
...
main() {
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        ...
        if selected_skill "capture-gotcha"; then
            echo "capture-gotcha 的正式记忆写入 ~/.agents/gotchas.md；项目记忆写入项目 .agents/gotchas.md。"
            echo "建议在核心提示词加一行：开工前先 recall capture-gotcha 正式记忆"
            echo ""
        fi
