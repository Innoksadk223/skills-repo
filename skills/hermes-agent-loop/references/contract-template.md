...
> 复制到 `state/[task-slug]/state.md`。Hermes 唯一必写状态文件。
...
- 执行模式：[Hermes executor | CC executor]
...
- `review.md` 最后 AUDIT 为 `ESCALATE_REPLAN`：Hermes 更新 Contract（重新分解步骤、调整范围），然后重新进入 ACT。连续两次 `ESCALATE_REPLAN` 无进展：升级到 USER_GATE。
- `review.md` 最后 AUDIT 为 `PROCEED_TO_VERIFY`：进入 VERIFY。
...
- blocker、硬上限或低收益：停止并汇报。

## Baseline

<!-- VERIFY 通过后由 Hermes 追加；不得改变交付物 -->
