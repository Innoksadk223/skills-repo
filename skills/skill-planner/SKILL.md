---
name: skill-planner
description: Use when starting any user task where skills, plugins, MCP tools, or "no specialized capability" must be chosen before acting.
---

# Skill Planner

启动时的轻量路由器：只决定本轮是否需要专项能力、工具或“无需专项能力”，以及它们的调用顺序。输出路由后继续执行；不要把路由变成计划、解释或确认环节。

## 快速流程

1. 判断用户真正要达成的结果，不只看表面命令
2. 扫描当前已注入的 skills / plugins / MCP tools
3. 只选择会改变执行路径、质量门槛、工具可用性或安全边界的真实能力
4. 按实际执行顺序输出路由；隐藏 `skill-planner` 自身
5. 立刻进入下一步：执行、澄清，或调用被选中的能力

## 输出格式

需要 `skill-planner` 之外的专项能力 / plugin / MCP tool：

`skill-planner` 是内部路由器：必须调用，但永远不作为“📋 任务路由”的条目展示；编号从第一个其它必要能力开始。

```
📋 任务路由
① real-capability-name → 本轮负责什么
② real-capability-name → 本轮负责什么
```

不需要 `skill-planner` 之外的专项能力：

```
📋 任务路由
无需专项 skill，直接执行
```

## 核心规则

- **少列**：只列必要项；“可能有用”“也许相关”不算必要。
- **真实名称**：路由条目必须是当前已注入的真实 skill / plugin / MCP tool 名称；内部可按能力角色判断，对外不写泛称，不编造不存在的能力。
- **隐藏自身**：`skill-planner` 只负责生成路由，不列入用户可见路由；即使用户点名或任务对象是 `skill-planner`，也只调整路由判断。
- **用户点名**：用户点名的可用能力默认优先纳入；若能力不存在、明显偏离真实目标、触发安全/不可逆边界，或点名的是 `skill-planner` 自身，则不列并简短说明。
- **硬门槛必列**：文件类型、领域、工具、执行安全或协作顺序明确要求的能力必须列；完成前验证是否发生不由本技能调度。
- **顺序**：先列会校准方向或边界的外层能力，再列产物/领域/工具能力；只表达调用顺序，不展开流程。
- **不等确认**：路由后直接开始；用户可随时纠正。
- **不展开解释**：必要项全部列出，每项一行；不为凑完整而补无关项。
- **不复述清单**：不要把已注入能力列表搬给用户，只输出本轮会实际调度的项。
- **不维护关键词表**：依靠当前可用能力和任务目标推理。

## 边界判断

- **无需专项能力**：单步读文件、搜代码、查命令、解释概念。
- **需要专项能力**：多步骤产物、代码/行为修改、调试、评审、论文/PPT/表格/文档、联网核验、浏览器操作、用户点名可用能力。
- **需求不清**：若目标或验收标准会改变路线，先问一个具体问题；若只是不影响方向的细节，明确假设后继续。
- **能力缺失**：用户点名但当前未注入时，不列入路由；简短说明不可用，再选择可用替代、直接执行或追问。

## 常见误判

- **把所有相关 skill 都列上**：只列会实际调度的能力。
- **用户点名 `skill-planner` 就展示它**：不展示自身，只展示其它执行能力。
- **把能力角色当名称展示**：用户可见路由只能写真实已注入名称。
- **把路由写成计划**：每项一行，不展开步骤。
- **不确定就停住**：只有路线会因此改变时才问。

## 原则

路由是开工前的导航，不是计划书。准确优先，简短其次；能不列就不列，关键项不能漏。用户看到的是接下来会被实际调度的专项能力，不是内部思考过程。
