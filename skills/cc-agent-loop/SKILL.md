...
9. **OPTIMIZE_LOOP**: mandatory triage after baseline. Scan for optimization candidates across four dimensions: functionality (missing or unnecessary features), conciseness (trim redundancy for token efficiency), maintainability (naming, structure, reuse), usability (beginner-friendly, fewer footguns). If any qualify, primary CC executes them and audit CC re-verifies. Skip only when no candidates exist.
