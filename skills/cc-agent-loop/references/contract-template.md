...
> 复制到 `state/[task-slug]/state.md`。主 CC 唯一必写状态文件；审查 CC 只读它，不在其中写审查结论。
...
- 审查 Agent 作用域：[独立 CC 审查进程；启动时写入 `audit_session`，任务结束时清除]
...
- `review.md` 有 pending appeal：恢复同一审查 CC 裁决。
- `next` 指向未完成修正：继续 ACT，必要时先更新 Contract。
- `review.md` 最后 AUDIT 为 `ESCALATE_REPLAN`：主 CC 更新 Contract（重新分解步骤、调整范围），然后重新进入 ACT。连续两次 `ESCALATE_REPLAN` 无进展：升级到 USER_GATE。
- `review.md` 最后 AUDIT 为 `PROCEED_TO_VERIFY`：同一审查 CC 进入 VERIFY。
