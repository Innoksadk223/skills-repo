...
- Latest AUDIT is `ESCALATE_REPLAN` -> primary CC drafts contract update, presents to user for confirmation before modifying `state.md`. After approval, re-enter ACT. Two consecutive `ESCALATE_REPLAN` without progress -> report to user and stop.
...
Primary CC summarizes from `state.md` and `review.md`: changed, why, checklist result, risks, and next step. Do NOT clear `audit_session` or delete process files — only the user can authorize cleanup of `state.md` and process files. Keep only minimal `state/inbox.md` when unresolved items remain.
