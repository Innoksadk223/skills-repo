...
**handoff 条件 ≠ 验收标准。** handoff 只管"产出物存在且格式对"，不管"质量好不好"——质量交给 Evaluator。
...
3. 所有剩余无依赖步骤 → 一组 parallel batch（≤3）
4. 串行链内部按依赖顺序逐个分派 Worker agent

**并行 batch 失败处理：** batch 中任一 Worker 失败 → 等待 batch 全部完成（或超时），将失败步骤信息汇总后统一进入 Troubleshooter。
