...
**铁律：首轮先问，不直接 PLAN。** 先做苏格拉底式 Intake，确认目标、约束、取舍和验收标准（确保这些信息最终写入本地文件，如 plan 产物）；清楚后再输出 PLAN 并进入 ACT。不得只在验收模糊时才问，也不得自行猜测。这确保循环有清晰的终止条件。

## Socratic Intake（PLAN 前置）

首轮进入 agent-loop 时，Orchestrator 先向用户发起一轮简短追问：

1. **诊断当前理解**：用一句话说出你认为用户真正要的结果。
2. **点破关键模糊点**：指出会改变路线或验收的 1–3 个不确定性。
3. **追问取舍**：用具体选项或开放问题确认优先级、边界、质量标准。
4. **收敛后再计划**：用户回答后，把答案转成 Loop Contract 和可量化 checklist，再输出 PLAN。

追问只问会影响执行的问题；不把 Intake 变成访谈表。若用户已明确要求跳过询问，仍需在 PLAN 中显式写出假设和风险。
...
5. 并行步骤需确保无语义冲突（共享接口/数据格式变更）。Orchestrator 在 PLAN 阶段人工判定，必要时强制串行
