...
Full AUDIT/VERIFY/OPTIMIZE/FINAL_VERIFY formats, appeal protocol, and recovery routing for Codex.

## AUDIT

Audit subagent must be the same persistent Codex session for the entire task — spawned as a child thread, resumed via `codex exec resume "$AUDIT_SESSION_ID"` after interruption.
...
If `CONTINUE_FIX`, the primary Codex follows each `fix_instruction` or writes `appeal.md`.
...
Same audit Codex independently re-executes verification commands for each checklist item after `PROCEED_TO_VERIFY`. The audit Codex must not rely on evidence recorded during OBSERVE — it runs the commands itself and records the actual output.
...
Approximate per-phase token costs for the audit Codex process:

| Phase | Per-phase cost | Notes |
|-------|---------------|-------|
| AUDIT | ~2K-6K tokens | `--yolo` skips approval overhead |
| VERIFY | ~2K-5K tokens | Mostly Bash execution |
| FINAL_VERIFY | ~1K-3K tokens | Baseline check only |
| Total (typical) | <15K tokens | Stop condition is checklist completion |

If a phase nears its turn cap without resolution, report evidence and escalate — do not blindly raise the cap.
