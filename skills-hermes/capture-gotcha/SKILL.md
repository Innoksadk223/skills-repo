---
name: capture-gotcha
description: 当工具调用（terminal、browser、web、execute_code 等）因环境、路径、配置、权限、代理、版本、环境变量、端口或工具安装方式等通用原因出错时，提炼可复用教训并写入 ~/.hermes/env.md。用于沉淀稳定环境经验，避免重复踩坑。不适用：代码逻辑错误、业务规则错误、需求理解偏差、临时性外部服务故障。
---
